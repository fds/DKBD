 ...
 */
(function (w) {
    var server = databasepage.hosturl + "update/update.json",//获取升级描述文件服务器地址
    ...
    checkInterval = 0,//升级检查间隔，先暂定随时更新，后续版本稳定后再实现周期更新
    dir = null;
    ...
    function initUpdate() {
        ...
        plus.io.requestFileSystem(plus.io.PRIVATE_DOC, function (fs) {
            fs.root.getDirectory(localDir, { create: true }, function (entry) {
                ...
                alert("准备升级操作，打开update目录失败：" + e.message);
            });
        }, function (e) {
            alert("准备升级操作，打开doc目录失败：" + e.message);
        });
    }
